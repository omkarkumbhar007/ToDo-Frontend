...
function TrashIcon({ className }) {
  return (
    <svg className={className} width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      ...
      <path d="M6 9h12l-1 11a2 2 0 01-2 2H9a2 2 0 01-2-2L6 9z" fill="currentColor" />
    </svg>
  )
}
    ...
    <main className={styles.container}>
      ...
      <div className={styles.inputRow}>
        <input
          ...
          aria-label="New task"
        />
        <button onClick={addTask} className={styles.addBtn} aria-label="Add task">
          Add
        </button>
      </div>

      {isLoading && <p className={styles.loadingText}>Loading tasks…</p>}
      {error && <p className={styles.errorText}>Failed to load tasks. Please refresh.</p>}
      ...
      {!isLoading && !error && (
        <ul className={styles.taskList} aria-label="Tasks">
          {todos.map((t) => {
            ...
            return (
              <li key={t.id} className={styles.taskItem}>
                ...
                <div className={styles.actions} role="group" aria-label={`Actions for task ${t.id}`}>
                  {!isEditing ? (
                    <>
                      <button
                    ...
                    <>
                      ...
                      <button className={styles.cancelBtn} onClick={cancelEdit} aria-label="Cancel edit" title="Cancel">
                        Cancel
                      </button>
                    </>
                  )}
          ...
          {todos.length === 0 && <li className={styles.emptyState}>No tasks yet. Add your first task above.</li>}
        </ul>
      )}
        ...
        >
          <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              ...
              <button className={styles.iconBtn} aria-label="Close" onClick={() => setSelected(null)} title="Close">
                Close
              </button>
            </div>
            ...
            <div className={styles.modalBody}>
              ...
              {selected.category && (
                <div className={styles.detailRow}>
                  ...
                  <span className={styles.detailValue}>{selected.category}</span>
                </div>
              )}
              {selected.due_date && (
                <div className={styles.detailRow}>
                  <span className={styles.detailLabel}>Due Date</span>
                  <span className={styles.detailValue}>{selected.due_date}</span>
                </div>
              )}
              {selected.notes && (
                <div className={styles.detailRow}>
              ...
              {selected.created_at && (
                <div className={styles.detailRow}>
                  ...
                  <span className={styles.detailValue}>{selected.created_at}</span>
                </div>
              )}
              {selected.updated_at && (
                <div className={styles.detailRow}>
                  <span className={styles.detailLabel}>Updated</span>
                  <span className={styles.detailValue}>{selected.updated_at}</span>
                </div>
              )}
            </div>

            <div className={styles.modalFooter}>
              <button className={styles.addBtn} onClick={() => setSelected(null)} aria-label="Close details">
                Close
              </button>
            </div>
          </div>
